								...
								<table border=1  class="table">
									...
									<jsp:useBean id="empFuncSvc" scope="page" class="com.empfunc.model.EmpFuncService" />
										...
										<tr>
											...
											<td>
												<c:forEach var="funcVO" items="${funcSvc.all}" varStatus="nextLine">
													<input type="checkbox" name="func${nextLine.count}"
													${empFuncSvc.getByOne(empVO.emp_id,funcVO.func_id).func_id == funcVO.func_id ? 'checked':''}
													disabled />
													<label for="func${nextLine.count}"  ${empFuncSvc.getByOne(empVO.emp_id,funcVO.func_id).func_id == funcVO.func_id ? 'class=spotlight':'class=normal'}>
													${funcVO.func_name}
													</label>
												</c:forEach>
											</td>
